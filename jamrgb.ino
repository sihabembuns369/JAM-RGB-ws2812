...
void handlecerah() {
  ...
  EEPROM.write(44, pos);
  EEPROM.commit();
  FastLED.setBrightness(EEPROM.read(44));
...
void setting_warna() {
  klike = server.arg("warna");
  String efek1 = server.arg("warnaa");
  ...
  EEPROM.write(42, efek_1);
  EEPROM.write(43, klik);
  EEPROM.commit();
...
void ppp(int startindex, int number) {
  ...
  for (i = 0; i < 21; i++) {
    LEDs[i + startindex] = ((numbers[number] & 1 << i) == 1 << i) ? CHSV(hue + (i * 40), 255, 255) : alternateColor;
...
void loop() {
  ...
  else if (jamnya > 20 && jamnya <= 21) {
    hl = hl - hl;
    hr = hr + 8;
  } else if (jamnya > 21 && jamnya <= 24) {
    hl = hl - 1;
    hr = hr - 2;
  }
  ...
  if (EEPROM.read(42) == 1) {
    ...
    ppp(45, ml);
    ppp(66, mr);
  } else if (EEPROM.read(42) == 2 ) { //efek1 == 2 and klik == 0
    Serial.println("efek");
  ...
  } else if (EEPROM.read(42) == 0 ) { // klik == 0 and efek1 == 0
    Serial.println("warna");
  switch (EEPROM.read(43)) {
    case 1: //merah
  ...
  displayTime(66, mr, warnane);
  } else if (EEPROM.read(42) == 3) { //efek2
    ...
    displayTime(45, ml, warnane);
    displayTime(66, mr, warnane);
  } else if (EEPROM.read(42) == 4) { //efek campur(mix)

    if ((millis() - waktu4) >= 14000  && (millis() - waktu4) < 24000) {
  ...
  displayTime(66, mr, warnane);
    ...
    } else if ((millis() - waktu4) >= 24000 && (millis() - waktu4) < 34000) {
  ...
  Serial.println(alrm[now.DayOfWeek()]);

  Serial.println(now.DayOfWeek());
  Serial.println(nama_hari[now.DayOfWeek()]);
}
